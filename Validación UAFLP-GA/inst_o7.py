# Python Script para la optimización de UAFLP mediante GA (Instancias Meller et al 7 dptos)

from algGenetico_uaflp import *
import pandas as pd

# Extraer y preparar datos de entrada de 'inst_o7.xlsx'

data = pd.read_excel("inst_o7.xlsx")

# Flujos de materiales * Costos de manejo
flujos_cols = list(data.columns.values)
flujos_mats = np.array(data[flujos_cols[1:8]])
...
# cromo_dep = np.array([7, 5, 3, 2, 6, 4, 1])
# cromo_bay = np.array([0, 0, 1, 0, 0, 0, 1])

# Try best García-Hernández (2020)

# cromo_dep = np.array([3, 5, 7, 1, 4, 6, 2])
# cromo_bay = np.array([0, 0, 1, 0, 0, 0, 1])
...
# Aplicación del algoritmo genético para la instancia dada (x10 repeticiones)
