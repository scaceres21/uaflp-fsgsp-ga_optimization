# Python Script para la optimización de UAFLP mediante GA (Instancias Meller et al 9 dptos)

from algGenetico_uaflp import *
import pandas as pd


# Extraer y preparar datos de entrada de 'inst_o9.xlsx'

data = pd.read_excel("inst_o9.xlsx")

# Flujos de materiales * Costos de manejo
flujos_cols = list(data.columns.values)
flujos_mats = np.array(data[flujos_cols[1:10]])
...
# cromo_dep = np.array([7, 8, 4, 1, 2, 3, 6, 9, 5])
# cromo_bay = np.array([0, 1, 0, 0, 1, 0, 0, 0, 1])

# Try best García-Hernández (2020)

# cromo_dep = np.array([3, 1, 6, 9, 5, 4, 2, 7, 8])
# cromo_bay = np.array([0, 0, 0, 0, 1, 0, 1, 0, 1])
...
ga = AlgGenetico(tam_poblacion=200, num_generaciones=200, tam_torneo=8, prob_cruce=0.9, prob_mutacion=0.3)
