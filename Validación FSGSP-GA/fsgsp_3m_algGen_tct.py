# The flowshop group scheduling problem (Salmasi et al, 2010 (3-machine))
...
while reps < 6:
    ...
    job_cont = 1
    for gr in groups:
        n_jobs = np.random.randint(2, 4 + 1, 1)[0]
            ...
            for g in groups:
                if gro == g:
                    ...
                    if mac == 1:
                        ...
                        group_list.append(np.random.randint(17, 67, 1)[0])
                    elif mac == 3:
                        group_list.append(np.random.randint(45, 95 + 1)[0])
    ...
    ga_sched = AlgGeneticoSched(tam_poblacion=100, num_generaciones=100, tam_torneo=3,
...
df.to_excel("results_fsgsp.xlsx")
